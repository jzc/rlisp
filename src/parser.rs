...
impl<'s, 'm, 'a> Parser<'s, 'm, 'a> {
    pub fn new(tokens: Vec<Token<'s>>, mem: &'a mut Memory<'s, 'm>) -> Self {
        Parser {
            tokens: tokens,
            start: 0,
            current: 0,
            ...
            Some(token) => match token {
                ...
                Token::OpenParen => match self.peek() {
                    ...
                    _ => {
                        ...
                        loop {
                            ...
                            match self.expr() {
                                Ok(e) => {
                                    ...
                                    match tail {
                                        None => head = None, //Some(curr),
                                        Some(_) => {
                                            // let cell = curr.borrow_mut();
                                            unimplemented!()
                                        }
                                    }
                                    tail = Some(curr);
                        ...
                        match self.advance() {
                            Some(Token::ClosedParen) => Ok(SExpr::Ref(head.unwrap())),
                            _ => Err(ParseError { message: "Missing closing parenthesis", line: 0}),
                        }
                    }
                }
